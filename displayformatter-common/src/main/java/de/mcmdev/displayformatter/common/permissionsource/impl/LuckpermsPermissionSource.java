...
public class LuckpermsPermissionSource<P> extends AbstractPermissionSource<P> {
  ...
  public LuckpermsPermissionSource(DisplayFormatter<P> displayFormatter) {
    ...
    this.tabPrefixSuffixUseMeta =
        displayFormatter.getMainConfiguration().getBoolean("luckperms.tab-prefix-suffix-use-meta");
    this.chatPrefixSuffixUseMeta =
        displayFormatter.getMainConfiguration().getBoolean("luckperms.chat-prefix-suffix-use-meta");
        ...
        .subscribe(
            ...
            userDataRecalculateEvent -> {
              P player =
                  displayFormatter
                      .getPlatform()
                      .getPlayer(userDataRecalculateEvent.getUser().getUniqueId());
              if (player == null) return;
              displayFormatter.getDisplayDataProvider().invalidatePlayerCaches(player);
                  ...
                  .ifPresent(
                      team -> {
                        displayFormatter
                            ...
                            .forEach(
                                all -> {
                                  displayFormatter.getPlatform().deleteTeam(player, team.getName());
                                });
                        displayFormatter.getTeamManager().unregisterTeam(player);
                      });

              Team<P> newTeam = displayFormatter.getTeamManager().createAndRegisterTeam(player);
              ...
              displayFormatter
                  ...
                  .forEach(
                      all -> {
                        displayFormatter.getPlatform().createTeam(all, newTeam);
                      });
            });
                  ...
                  .filter(
                      user ->
                          user.getInheritedGroups(QueryOptions.defaultContextualOptions())
                              .contains(groupDataRecalculateEvent.getGroup()))
                  .forEach(
                      user -> {
                        P player = displayFormatter.getPlatform().getPlayer(user.getUniqueId());
                        if (player == null) return;
                        displayFormatter.getDisplayDataProvider().invalidatePlayerCaches(player);
                                  ...
                                  displayFormatter
                                      ...
                                      .forEach(
                                          all -> {
                                            displayFormatter
                                                .getPlatform()
                                                .deleteTeam(player, team.getName());
                                          });
                                  displayFormatter.getTeamManager().unregisterTeam(player);
                                });
        ...
        .map(
            user ->
                tabPrefixSuffixUseMeta
                    ? user.getCachedData().getMetaData().getMetaValue("tab-prefix")
  ...
  public Optional<String> getTabSuffix(UUID uuid) {
    return getUser(uuid)
        .map(
            user ->
                tabPrefixSuffixUseMeta
                    ? user.getCachedData().getMetaData().getMetaValue("tab-suffix")
                    : user.getCachedData().getMetaData().getSuffix());
  }

  @Override
  public Optional<String> getTabColor(UUID uuid) {
    return getUser(uuid).map(user -> user.getCachedData().getMetaData().getMetaValue("tab-color"));
  }
        ...
        .map(
            user ->
                chatPrefixSuffixUseMeta
                    ? user.getCachedData().getMetaData().getMetaValue("chat-prefix")
                    : user.getCachedData().getMetaData().getPrefix());
  }
  ...
  public Optional<String> getChatSuffix(UUID uuid) {
    return getUser(uuid)
        .map(
            user ->
                chatPrefixSuffixUseMeta
                    ? user.getCachedData().getMetaData().getMetaValue("chat-suffix")
                    : user.getCachedData().getMetaData().getSuffix());
  }

  @Override
  public Optional<String> getChatColor(UUID uuid) {
    return getUser(uuid).map(user -> user.getCachedData().getMetaData().getMetaValue("chat-color"));
  }
